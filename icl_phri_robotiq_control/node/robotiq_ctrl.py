...
class RobotiqGripperCtrl:
    def __init__(self):
        ...
        self.query_sub = rospy.Subscriber('CModelRobotInput', inputMsg.CModel_robot_input, self.query_callback)
        self.command_pub = rospy.Publisher('CModelRobotOutput', outputMsg.CModel_robot_output, queue_size=1)
        self.reset()
        sleep(1)
        self.activate()
        sleep(3)
...
class ForceTorqueSensor:
    def __init__(self, topic_name):
        self.fts_sub = rospy.Subscriber(topic_name, WrenchStamped, self.fts_callback)
        self.header = Header()
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if force_torque_sensor.get_force()[2] < -10.0:
            gripper_ctrl.set_speed(150)
            gripper_ctrl.close_gripper()
            print 'Closing fripper'
        if force_torque_sensor.get_force()[2] > 5.0:
            gripper_ctrl.set_speed(150)
            print 'curr_pos:' + str(gripper_ctrl.curr_pos)
            gripper_ctrl.close_gripper(gripper_ctrl.curr_pos - 5)
            sleep(0.5)
            gripper_ctrl.close_gripper(gripper_ctrl.curr_pos - 10)
            gripper_ctrl.open_gripper()
            print 'Opening fripper'
        rate.sleep()
